...
namespace Workflows.ObslugaWiadomosci
{
    public sealed partial class ObslugaWiadomosci : SequentialWorkflowActivity
    {
        ...
        {
            Debug.WriteLine("Proc: Mail_Setup_ExecuteCode");
            mail = new MailMessage();
            isMailReadyToSend = false;

            string defautlEmailSender = BLL.admSetup.GetValue(item.Web, "EMAIL_DEFAULT_SENDER");
            ...
            if (item["colNadawca"] != null)
            {
                ...
                if (mail.From.Address.ToString().Equals(defautlEmailSender))
                {
            ...
            Debug.WriteLine("mail.From=" + mail.From.ToString());
            ...
            if (item["colOdbiorca"] != null && !string.IsNullOrEmpty(item.Title))
            {
                ...
                if (isKopiaDoNadawcy)
                {
                    if (!item["colNadawca"].ToString().Equals(defautlEmailSender))
                    {
